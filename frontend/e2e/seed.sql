-- E2E real-backend テスト用シードデータ
-- 実行前提: バックエンドコンテナが起動済みであること
--
-- 使い方（ホストから docker exec 経由で実行）:
--   docker exec -i auth-mysql sh -c "mysql -u root -p'docker#DOCKER1234' authorization" < frontend/e2e/seed.sql
--
-- ・auth/me と notifications は常にモックのため、staffs/notifications の件数は任意
-- ・clients: mockClients / mockClientDetail / mockClientsWithDeleted の値に一致させる
-- ・invitations: token='e2e-tkn-001' を使う（invitation.spec.ts の期待値・13文字以内で省略されない）

SET FOREIGN_KEY_CHECKS = 0;
TRUNCATE TABLE notifications;
TRUNCATE TABLE invitations;
TRUNCATE TABLE clients;
TRUNCATE TABLE staffs;
SET FOREIGN_KEY_CHECKS = 1;

-- staffs （mockStaffs に合わせる）
INSERT INTO staffs
  (id, name, email, provider, provider_id, avatar, role, last_login_at, created_at, created_by, updated_at, updated_by, version)
VALUES
  (1, '田中 太郎', 'tanaka.taro@example.com',   1, 'e2e-provider-001', NULL, 1, '2026-01-01 00:00:00', '2026-01-01 00:00:00', 0, '2026-01-01 00:00:00', 0, 1),
  (2, '佐藤 花子', 'sato.hanako@example.com',   1, 'e2e-provider-002', NULL, 2, '2026-02-01 00:00:00', '2026-02-01 00:00:00', 0, '2026-02-01 00:00:00', 0, 1);

-- clients
--   id=1: 株式会社アルファテック  status=2(利用中)  → mockClientDetail に一致
--   id=2: ベータソリューションズ株式会社  status=1(準備中)
--   id=3: ガンマシステム株式会社  status=4(archived) → mockSoftDeletedClientDetail に一致
SET @priv = '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----';

SET @pub  = '-----BEGIN PUBLIC KEY-----\\npQIDAQAB\n-----END PUBLIC KEY-----';

INSERT INTO clients
  (id, name, identifier, post_code, pref, city, address, building, tel, email,
   access_token, private_key, public_key, fingerprint,
   status, start_at, stop_at,
   created_at, created_by, updated_at, updated_by, deleted_at, version)
VALUES
  (1, '株式会社アルファテック', 'e2e-client-001', '1000001', '東京都', '千代田区', '丸の内1-1-1', '丸の内ビル3F', '0310000001', 'info@alpha-tech.example.com',
   'e2eaccesstoken00000000000000000000000000000000000000000000000001',
   @priv, @pub, 'SHA256:test',
   2, '2026-01-01 09:00:00', NULL,
   '2026-01-01 09:00:00', 0, '2026-01-01 09:00:00', 0, NULL, 1),

  (2, 'ベータソリューションズ株式会社', 'e2e-client-002', '1500001', '東京都', '渋谷区', '渋谷2-2-2', '', '0310000002', 'info@beta-solutions.example.com',
   'e2eaccesstoken00000000000000000000000000000000000000000000000002',
   @priv, @pub, 'SHA256:test',
   1, '2026-01-02 09:00:00', NULL,
   '2026-01-02 09:00:00', 0, '2026-01-02 09:00:00', 0, NULL, 1),

  (3, 'ガンマシステム株式会社', 'e2e-client-003', '1600001', '東京都', '新宿区', '新宿3-3-3', '新宿タワー10F', '0310000003', 'info@gamma-system.example.com',
   'e2eaccesstoken00000000000000000000000000000000000000000000000003',
   @priv, @pub, 'SHA256:test',
   4, '2026-01-03 09:00:00', '2026-12-31 18:00:00',
   '2026-01-03 09:00:00', 0, '2026-01-03 09:00:00', 0, '2027-01-01 00:00:00', 1);

-- invitations（invitation.spec.ts の e2e-tkn-001 に一致させる）
INSERT INTO invitations
  (token, created_at, created_by, updated_at, updated_by)
VALUES
  ('e2e-tkn-001', '2026-01-01 00:00:00', 1, '2026-01-01 00:00:00', 1);
