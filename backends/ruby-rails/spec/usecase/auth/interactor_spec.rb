...
RSpec.describe UseCase::Auth::Interactor do
  ...
  describe "#find_user" do
    it "returns Vo when staff found" do
      ...
      vo = described_class.new(stub_repo).find_user(5)
  ...
  describe "#login" do
    let(:dto) { UseCase::Auth::LoginDto.new(provider: 1, provider_id: "new-id", name: "New User", email: "new@example.com", avatar: nil) }
    ...
    it "updates existing staff and returns Vo" do
      ...
      vo = described_class.new(stub_repo).login(login_dto)
      expect(vo.id).to eq 5
