...
impl Repository for SqlxNotificationRepository {
    async fn list_page(
        ...
        let mut qb: QueryBuilder<sqlx::MySql> =
            QueryBuilder::new("SELECT * FROM notifications WHERE staff_id = ");
    ...
    async fn bulk_mark_read(
        ...
        let mut qb: QueryBuilder<sqlx::MySql> =
            QueryBuilder::new("UPDATE notifications SET `read` = true, updated_at = ");
        qb.push_bind(now);
    ...
    async fn patch(&self, id: i64, read: bool) -> Result<bool, DomainError> {
        let now = chrono::Utc::now();
        let result =
            sqlx::query("UPDATE notifications SET `read` = ?, updated_at = ? WHERE id = ?")
                .bind(read)
                .bind(now)
                .bind(id)
                .execute(&self.pool)
