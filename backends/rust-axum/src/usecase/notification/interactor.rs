...
impl Interactor {
    ...
    pub async fn list_page(
        ...
        let entity_page = self
            .repo
            .list_page(staff_id, cursor.as_deref(), limit)
            .await?;
        let items = entity_page.items.into_iter().map(to_item).collect();
        Ok(Page {
            items,
            next_cursor: entity_page.next_cursor,
        })
    }
    ...
    pub async fn bulk_mark_read(&self, staff_id: u32) -> Result<i64, UseCaseError> {
        Ok(self
            .repo
            .bulk_mark_read(staff_id as i64, vec![], true)
            .await?)
    }
...
mod tests {
    ...
    impl Repository for MockNotifRepo {
        async fn list_page(
            ...
            _: Option<&str>,
            _: i32,
        ) -> Result<EntityPage, DomainError> {
            Ok(self
                .list_page_result
                .lock()
                .unwrap()
                .take()
                .unwrap_or(EntityPage {
                    items: vec![],
                    next_cursor: None,
                }))
        }
        ...
        async fn patch(&self, _: i64, _: bool) -> Result<bool, DomainError> {
            Ok(true)
        }
    }
    ...
    impl StaffRepository for MockStaffRepo {
        async fn count_by_condition(&self, _: Condition) -> Result<i64, StaffDomainError> {
            Ok(0)
        }
        async fn find_by_condition(&self, _: Condition) -> Result<Vec<Staff>, StaffDomainError> {
            Ok(vec![])
        }
        async fn find_by_id(&self, id: u32) -> Result<Option<Staff>, StaffDomainError> {
            Ok(Some(make_staff(id)))
        }
        async fn find_by_provider(
            &self,
            _: i32,
            _: &str,
        ) -> Result<Option<Staff>, StaffDomainError> {
            Ok(None)
        }
        async fn find_all_active(&self) -> Result<Vec<Staff>, StaffDomainError> {
            Ok(self.active_staffs.lock().unwrap().drain(..).collect())
        }
        async fn save(&self, s: Staff) -> Result<Staff, StaffDomainError> {
            Ok(s)
        }
        async fn update_role(
            ...
            _: u32,
            _: i32,
        ) -> Result<bool, StaffDomainError> {
            Ok(true)
        }
        async fn soft_delete(&self, _: u32, _: u32, _: i32) -> Result<bool, StaffDomainError> {
            Ok(true)
        }
        async fn restore(&self, _: u32) -> Result<bool, StaffDomainError> {
            Ok(true)
        }
    }
