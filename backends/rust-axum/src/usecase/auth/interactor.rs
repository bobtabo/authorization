...
use crate::domain::staff::{entity::Staff, repository::Repository, value_objects::Vo};
use std::sync::Arc;
...
impl Interactor {
    ...
    pub async fn login(&self, dto: LoginDto) -> Result<Vo, UseCaseError> {
        ...
        let existing = self
            .staff_repo
            .find_by_provider(dto.provider, &dto.provider_id)
            .await?;
...
mod tests {
    ...
    impl Repository for MockStaffRepo {
        ...
        async fn find_by_condition(&self, _: Condition) -> Result<Vec<Staff>, DomainError> {
            Ok(vec![])
        }
