...
pub async fn index(
    ...
    let dto = ListConditionDto {
        ...
        start_to: q.start_to,
        statuses: int_array_query(raw_query.as_deref(), "statuses"),
        offset,
        limit,
        sort: q.sort,
...
/// クライアント詳細を返します。
pub async fn show(State(state): State<AppState>, Path(id): Path<u64>) -> (StatusCode, Json<Value>) {
...
pub async fn store(
    ...
    let activate_url = format!(
        "{}/clients/{}/qr",
        state.cfg.app.frontend_url, result.identifier
    );
