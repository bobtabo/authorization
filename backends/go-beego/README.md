...
&nbsp;&nbsp;
<a href="https://beego.vip/" target="_blank"><img src="https://avatars.githubusercontent.com/u/3884041" height="72" alt="Beego"></a>
</p>
...
認可サーバー API の **Go / Beego** バックエンド実装です。
...
Handler (internal/handler/)
    │  リクエスト解析・レスポンス整形（beecontext.Context）
    ▼
...
backends/go-beego/
├── conf/
│   └── app.conf                 # Beego サーバー設定（ポート・実行モード等）
├── internal/
...
│   │   └── db/                  # GORM 接続
│   ├── handler/                 # Beego ハンドラー層（beecontext.Context）
...
| パッケージ | 用途 |
|---|---|
| `beego/beego/v2` | HTTP フレームワーク・ORM |
...
```bash
# docker/ ディレクトリから実行
bin/docker-go-beego.sh up    # 起動
bin/docker-go-beego.sh down  # 停止
bin/docker-go-beego.sh exec  # コンテナに入る
```
