...
func TestAdminInvitation_Index(t *testing.T) {
	truncateTables(t)

	t.Run("現在の招待URLが取得できる", func(t *testing.T) {
		inv := createInvitation(t, "current-test-token")
		w := do(http.MethodGet, "/api/admin/invitation", nil)
...
func TestAdminInvitation_Issue(t *testing.T) {
	...
	t.Run("招待URLが発行できる", func(t *testing.T) {
		w := do(http.MethodGet, "/api/admin/invitation/issue", nil, withCookie("staff_id", signStaffCookie(1)))
		if w.Code != http.StatusOK {
			t.Errorf("want 200, got %d: %s", w.Code, w.Body.String())
		}
		body := parseBody(w)
