...
namespace Authorization.Api.Infrastructure.Mail;

/// <summary>メール送信インターフェースです。</summary>
...
public sealed class SesMailer(MailSettings mail, AwsSettings aws, ILogger<SesMailer> logger) : IMailer
{
    public async Task SendActivationAsync(string to, string clientName, string activateUrl, CancellationToken ct = default)
    {
        ...
        var subject = MailSubject($"【{mail.AppName} / C#】ご利用開始のご案内", mail.AppEnv);
        var body    = BuildActivationHtml(clientName, activateUrl, mail.AppName);

        try
        {
            using var ses = BuildSesClient();
            var request = new SendEmailRequest
            {
                Source      = $"{mail.AppName} <{mail.FromAddress}>",
        ...
        {
            logger.LogError(e, "mail send error");
        }
    }
    ...
    private AmazonSimpleEmailServiceClient BuildSesClient()
    {
        ...
        if (!string.IsNullOrWhiteSpace(aws.Endpoint)) config.ServiceURL = aws.Endpoint;

        return string.IsNullOrWhiteSpace(aws.AccessKey)
            ? new AmazonSimpleEmailServiceClient(config)
            : new AmazonSimpleEmailServiceClient(new BasicAWSCredentials(aws.AccessKey, aws.SecretKey), config);
    }
    ...
    public static string MailSubject(string subject, string appEnv)
    {
        ...
        return label.Length > 0 ? $"[{label}]{subject}" : subject;
    }
