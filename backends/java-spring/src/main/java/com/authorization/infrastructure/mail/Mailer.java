...
public class Mailer {
     ...
     */
    public void sendActivation(String to, String clientName, String activateUrl) {
        if (to == null || to.isBlank()) {
            return;
        }
        String subject = mailSubject("【" + mailCfg.appName() + " / Java】ご利用開始のご案内");
        String body = buildActivationHtml(clientName, activateUrl, mailCfg.appName());

        try (SesClient ses = buildSesClient()) {
    ...
    private SesClient buildSesClient() {
        ...
        if (awsCfg.endpoint() != null && !awsCfg.endpoint().isBlank()) {
            builder = builder.endpointOverride(URI.create(awsCfg.endpoint()));
        }
        if (awsCfg.accessKey() != null && !awsCfg.accessKey().isBlank()) {
            builder = builder.credentialsProvider(StaticCredentialsProvider.create(
                    AwsBasicCredentials.create(awsCfg.accessKey(), awsCfg.secretKey())));
        }
