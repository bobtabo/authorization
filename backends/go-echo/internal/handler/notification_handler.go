...
func NewNotificationHandler(db *ent.Client, newNotifUC func(*ent.Client) *unotification.Interactor, cfg *config.Config) *NotificationHandler {
...
func (h *NotificationHandler) Index(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, map[string]interface{}{
		"items": mapNotificationItems(page.Items), "next_cursor": page.NextCursor,
	})
}
...
func (h *NotificationHandler) ReadAll(c echo.Context) error {
	...
	if txErr := withTx(c.Request().Context(), h.db, func(tx *ent.Tx) error {
		var e error
		updated, e = h.newNotifUC(tx.Client()).BulkMarkRead(unotification.BulkMarkReadDto{StaffID: staffID})
		return e
	}); txErr != nil {
		return txErr
	}
...
func mapNotificationItems(items []*domnotification.Item) []map[string]interface{} {
	...
	for _, n := range items {
		out = append(out, map[string]interface{}{
			...
			"title": n.Title, "message": n.Message, "url": n.URL, "read": n.Read,
