...
func TestNotification_Counts(t *testing.T) {
	...
	t.Run("通知件数が取得できる", func(t *testing.T) {
		...
		if err := testDB.Notification.UpdateOneID(n2.ID).SetRead(true).Exec(context.Background()); err != nil {
...
func TestNotification_Read(t *testing.T) {
	...
	t.Run("既読済みの自分の通知は200になる", func(t *testing.T) {
		...
		if err := testDB.Notification.UpdateOneID(n.ID).SetRead(true).Exec(context.Background()); err != nil {
			t.Fatalf("set read: %v", err)
		}
