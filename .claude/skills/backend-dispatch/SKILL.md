...
12言語・フレームワークで実装している（マイクロサービス分割ではない）。
...
| Java, Spring Boot | `backends/java-spring/` | `docker/bin/docker-java.sh` | `docker/local/app-java/` | `auth-java` | `java` | `bash` |
...
- 「PHPで」「Pythonで」「Rustで」「Kotlinで」「TypeScriptで」「C#で」「Javaで」は各1実装のみなので一意。
- 「全バックエンドで」「12言語すべてで」→ 上表の12ディレクトリすべてに同じ変更を展開する。
...
同一仕様の実装が12個あるため、仕様に関わる修正（OpenAPI・共通の振る舞い）は
